$Global:myPslibFilePath = @{
    psLibFilePath = 'E:\DataIn\gitRemoteProject\new_powershell_profile\bin\psLib.ps1'
    aliasFilePath = 'E:\DataIn\gitRemoteProject\new_powershell_profile\setting\alias.csv'
}

$Global:WebNetUrlUsual = @{
    stackedit    = 'https://stackedit.io/'
    myGithub     = 'https://myfinalday.github.io/'
    codepen      = 'https://codepen.io/'
    freeLicense  = 'http://jetbrains.license.laucyun.com/'
    iconfont     = 'http://www.iconfont.cn/search/index'
    webstormHelp = 'https://www.jetbrains.com/help/webstorm/2017.3/meet-webstorm.html'
}

$Global:ProgramPathHash = @{
    webstorm64     = 'C:\Users\vipyo\AppData\Local\JetBrains\Toolbox\apps\WebStorm\ch-0\181.5281.31\bin\webstorm64.exe'
    weChat         = 'C:\Users\Public\userSoft\WeChat\WeChat.exe'
    chrome         = 'C:\Program Files (x86)\Google\Chrome\Application\chrome.exe'
    powershell     = 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe'
    firefox        = 'C:\Program Files\Mozilla Firefox\firefox.exe'
    gvim           = 'C:\Users\Public\userSoft\Vim\vim81\gvim.exe'
    cmd            = 'C:\Windows\system32\cmd.exe'
    vim            = 'C:\Users\Public\userSoft\Vim\vim81\vim.exe'
    gitBash        = 'C:\Program Files\Git\git-bash.exe'
    i_view64       = 'C:\Users\Public\userSoft\IrfanView\i_view64.exe'
    wechatdevtools = 'C:\Users\Public\userSoft\weChatwebDev\??web?????.exe'
    BaiduNetdisk   = 'C:\Users\vipyo\AppData\Roaming\baidu\BaiduNetdisk\BaiduNetdisk.exe'
}

$Global:MyWorkPath = @{
    weChatTmp      = 'C:\Users\vipyo\Documents\WeChat Files\wxid_fodh1nkdqos422\Files'

    dengbangtiming = @{
        images = 'C:\Users\vipyo\workFor\newProject\dengbangtiming\wxapp\src\images'
    }
    bubuweiying    = @{
        oldDomain = 'http://bbwy.lwf.gzqichang.com/'
    }
}

$Global:MiniProgram = @{
    username = 'wxid_fodh1nkdqos422'
    AppId    = 'wx97807883a836a66d';
}

$Global:DocumentNetUrl = @{
    Vue         = 'https://cn.vuejs.org/v2/api/'
    MiniProgram = 'https://developers.weixin.qq.com/miniprogram/dev/api/'
    AntMobile   = 'https://antd-mobile.gitee.io/docs/react/introduce-cn'
    AntDesign   = 'http://ant-design.gitee.io/docs/react/introduce-cn'
    dva         = 'https://github.com/dvajs/dva/blob/master/README_zh-CN.md'
    wepy        = 'https://tencent.github.io/wepy/document.html#/'
    weui        = 'https://weui.io/'
    VueDesign   = 'http://tangjinzhou.gitee.io/ant-design-vue/docs/vue/introduce-cn/'
}

$Global:DevelopNetUrl = @{
    weixinDevelop = 'https://developers.weixin.qq.com/'
    weixinQa      = 'https://developers.weixin.qq.com/miniprogram/dev/qa.html'
}

$Global:ForTestApi = @{
    bitCoin    = 'https://api.coindesk.com/v1/bpi/currentprice.json'
    zhihuribao = 'https://news-at.zhihu.com/api/4/news/latest'
    zhihu      = 'https://github.com/izzyleung/ZhihuDailyPurify/wiki/%E7%9F%A5%E4%B9%8E%E6%97%A5%E6%8A%A5-API-%E5%88%86%E6%9E%90'
    gushi      = 'https://api.gushi.ci/all.json'
}